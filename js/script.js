...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const purchaseSubmitButton = document.querySelector('#purchase-submit-button');
    ...
    function displayProducts(category) {
        ...
        if (selectedCategory) {
            selectedCategory.items.forEach(product => {
                ...
                // Обработчик для кнопки "Buy It" в каждой карточке товара
    ...
    registrationFormElement.addEventListener('submit', function (event) {
        ...
        displayRegistrationData(registrationData);
        registrationForm.classList.add('hidden');
        purchaseSubmitButton.classList.remove('hidden');
    });

    purchaseSubmitButton.addEventListener('click', () => {
        purchaseSubmitButton.textContent = 'Thank you for your purchase!';
        setTimeout(() => {
            purchaseSubmitButton.textContent = 'Submit';
            purchaseSubmitButton.classList.add('hidden');
            purchaseSuccessMessage.classList.add('active');
            setTimeout(() => {
                purchaseSuccessMessage.classList.remove('active');
                displayCategories();
            }, 1500);
        }, 1500);
    });
