...
document.getElementById("runButton").addEventListener("click", function () {
    ...
    if (!isNaN(length) && length > 0) {
        ...
        output("Task 1: Create an array, its length and elements specified by the user.");
        output("User array: " + userArray);
        output("Task 2: Sort the array in ascending order.");
        userArray.sort((a, b) => a - b);
        output("Sorted array: " + userArray);
        output("Task 3: Remove elements from the array from 2 to 4 (inclusive).");
        if (userArray.length >= 5) {
            userArray.splice(1, 4);
            output("Updated array: " + userArray);
        } else {
            output("Array length is less than 5. Can't remove elements.");
        }
    }
});
